# -*- coding: utf-8 -*-
"""공공데이터포털 「전국초중등학교위치표준데이터」(publicDataPk=15021148)를
로그인·API키 없이 내려받아 tools/schools_raw.json 으로 저장.

방법: ① columList.json 으로 totalCount·svcTableNm·컬럼 목록 획득
      ② standard.json 으로 전체 레코드 JSON 다운로드 (page는 1부터)
"""
import json
import sys
import urllib.parse
...
PK = "15021148"
OUT = Path(__file__).resolve().parent / "schools_raw.json"
